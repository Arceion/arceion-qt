...
class GetStarted(QScrollArea):
	...
	def updateTheme(self):
		if self.loaded:
			...
			cards = [
				self.getting_started_card, self.next_steps_card, self.documentation_card
			]
			for card in cards:
				...
				""")
				if card.icon_char:
					card.icon_label.setPixmap(card.icon_char.update(
						color=self.AppTheme.colors.text.name(),
					).toPixmap())
				LabelStyle.label.update(
					color=self.AppTheme.colors.text.name(),
			...
			for item in self.documentation_card.documentation_list:
				item[0].setPixmap(Icons.Outlined.arrow_forward.update(
					color=self.AppTheme.colors.text.name(),
				).toPixmap())
				LabelStyle.label.update(
					color=self.AppTheme.colors.text.name(),
