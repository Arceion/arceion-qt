...
class ModelAbstract(Generic[T]):
    ...
    def _convertDict(cls, value: Any, dateType: type) -> dict:
        ...
        if not hasattr(dateType, "__args__"):
            return value
        return {key: cls._convert(item, dateType.__args__[1]) for key, item in value.items()}
    ...
    def _convertList(cls, value: Any, dateType: type) -> list:
        ...
        return [
            cls._convert(
                item,
                (next(iter(dateType.__args__)) if hasattr(dateType, "__args__") else Any),
            )
            for item in value
        ]
    ...
    def _json(self, data):
        ...
        if isinstance(data, dict):
            return {self._json(k): self._json(v) for k, v in data.items()}
        return data
